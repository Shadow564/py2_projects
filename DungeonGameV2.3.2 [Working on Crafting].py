...
class recipe(object):
  ...
  def add_recipe(self):
    ...
    list_of_recipes_readable.append(self)
    
def scan_for_truth(what_wanted):
    dusted = False
    for lel in list_of_recipes:
      if lel.endresult == what_wanted:
        dusted = True
        global inv
        yes = False
        bueno = True
        for part in lel.parts:
          for ele in inv:
            if ele == part:
              yes = True
            else:
